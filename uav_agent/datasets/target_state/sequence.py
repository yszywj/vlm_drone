...
class TargetStateSequence:
    ...
    def __post_init__(self) -> None:
        ...
        if any(item.association_review_required for item in frames):
            raise ValueError("sequence contains an unresolved offline association")
    ...
    *,
    ...
    for key, values in sorted(groups.items()):
        ...
        for reference_index in range(history_size, len(ordered)):
            ...
            # Keep uncertain observations in the timeline: removing individual
            # rows here would silently bridge across an identity/occlusion gap.
            if any(item.association_review_required for item in frames):
                continue
