    ...
    *,
    ...
    for record in records:
        ...
        if center is not None:
            ...
            if (not inside and label.center_in_image is not False
                    or label.center_in_image is not None and label.center_in_image != inside):
                errors.append(
                    "target center_pixel_uv is outside camera resolution for "
                    f"{record.frame_id}: center={center}, resolution={(width, height)}"
                )
            if label.center_in_image is not None:
                from datasets.target_state.projection import project_label_center
                projected, _ = project_label_center(label.position_world_m,
                    camera.position_world_m, camera.orientation_world_wxyz,
                    (camera.fx, camera.fy, camera.cx, camera.cy))
                if projected is None or any(abs(a-b) > 1e-3 for a,b in zip(projected, center)):
                    errors.append(f"target centre projection inconsistent with 3D label: {record.frame_id}")
