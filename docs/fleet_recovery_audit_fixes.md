...
- 可以识别需要协调的情况并安全升级（2026-09-16 时的边界）；也没有全机队联合提交或数学最小影响集合求解。
  - 2026-09-18 更新：配置开启后已支持有界 2~3 UAV 相关小组联合修复（两阶段 PREPARE/原子 PUBLISH 提交），仅限满足联合准入条件的 Linear V3 / GOTO peer；仍不支持任意运行中 Skill、Graph、大规模小组或全 Fleet 自动重规划。
